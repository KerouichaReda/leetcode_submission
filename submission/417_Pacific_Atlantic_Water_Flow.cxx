   ...
   public:
    ...
    bool check_p(vector<vector<int>>& matrix, int i, int j, int val) {
        ...
        if (val >= matrix[i][j] && !v_p[i][j]) {
            v_p[i][j] = true;
            return check_p(matrix, i - 1, j, matrix[i][j]) || check_p(matrix, i, j - 1, matrix[i][j]) ||
                   check_p(matrix, i + 1, j, matrix[i][j]) || check_p(matrix, i, j + 1, matrix[i][j]);
        }
        return false;
    }
    bool check_a(vector<vector<int>>& matrix, int i, int j, int val) {
        ...
        if (val >= matrix[i][j] && !v_a[i][j]) {
            v_a[i][j] = true;
            return check_a(matrix, i + 1, j, matrix[i][j]) || check_a(matrix, i, j + 1, matrix[i][j]) ||
                   check_a(matrix, i - 1, j, matrix[i][j]) || check_a(matrix, i, j - 1, matrix[i][j]);
        }
        return false;
    }
