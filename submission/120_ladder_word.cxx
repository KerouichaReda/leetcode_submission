...
#include <queue>
bool diff_one(std::string& str1, std::string& str2) {
...
std::vector<std::string> neighbors(std::string& str, std::vector<std::string>& _wordList) {
    std::vector<std::string> solution;
    for (std::string word : _wordList) {
        if (diff_one(str, word)) {
...
int ladder_length(std::string beginWord, std::string endWord, std::vector<std::string>& wordList) {
    std::unordered_set<std::string> visited(wordList.begin(), wordList.end());
    int solution{1};
    ...
    while (!q.empty()) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            visited.insert(word);
            std::vector<std::string> n = neighbors(word, wordList);
            for (std::string w : n) {
                if (!visited.count(w)) {
                    q.push(w);
                }
...
int ladderLength(std::string beginWord, std::string endWord, std::vector<std::string>& wordList) {
    ...
    while (!q.empty()) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            q.pop();
            if (word == endWord) {
                return solution;  // <-- if the solution found
            }
...
int main() {
    std::string beginWord = "sand";
    std::string endWord = "acne";
    ...
    std::cout << "The result is : " << ladderLength(beginWord, endWord, wordList) << std::endl;
    return EXIT_SUCCESS;
}