...
class Solution {
   public:
    int myAtoi(string str) {
        ...
        if ((length - zeros) >= 11) return (buffer[0] == '-') ? INT_MIN : INT_MAX;

        if ((length - zeros) >= 10) {
            if (buffer[0] == '-' && buffer[1] > '2') return INT_MIN;
            if (buffer[0] > '2') return INT_MAX;
        }
        ...
        if (buffer[0] == '-' && solution >= (unsigned int)INT_MAX + 1) return INT_MIN;
        if (solution >= (unsigned int)INT_MAX + 1) return INT_MAX;
